...
if (file_exists($fn))
{
	...
	$sql = "TRUNCATE TABLE kray";
	mysql_query ($sql) or die (mysql_error());
	$file = arrayFromHtml($fn);
	echo ' оличество записей в KRAY - ';
	echo count($file) . '<br />';
	foreach ($file as $k=> $item)
	{
		...
		$keyword = str_replace('&quot', '', $keyword);
		...
		$sql = "INSERT INTO kray (author, title, rubric, keyword, person, geograf, sourse)
